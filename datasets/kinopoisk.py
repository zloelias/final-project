...
pos = glob.glob("./data/kinopoisk/dataset/neu/*")

kinopoisk_dataset1 = load_dataset('text', data_files=neg)
kinopoisk_dataset1 = kinopoisk_dataset1.map(lambda x: {'text': x['text'], 'labels': -1, 'label_name': 'negative'})
...
kinopoisk_dataset3 = kinopoisk_dataset2.map(lambda x: {'text': x['text'], 'labels': 1, 'label_name': 'positive'})
